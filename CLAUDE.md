...
> **2026-08-24 起后端服务迁移**：今后新开发的后端服务都在 `~/Code/hsh-erp-rust`（Rust + axum + sqlx）。新开发后端服务的 API 文档一律维护在 `~/Code/hsh-erp-rust/docs/api/`（按域切分：`auth.md` / `users.md` / `delivery-notes.md` / `delivery-groups.md` / `websocket.md` / `index.md` 通用约定）；今后任何需要查后端接口的环节直接 `Read` 对应文件，不要去翻 `src/modules/*` 源码。
