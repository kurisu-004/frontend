...
7. **auth 域 2026-08-26 临时回滚 v1**：`/api/v1/auth/*` 是当前目标；新增 auth 功能统一走 `api`（v1）。**已知 trade-off**：v2 业务端点（**仅服务于 `src/views/delivery/DeliveryNoteScan.vue` 扫码建单页 + 其路由 B 间接依赖的 inspection 流程**，共 15 个 / 4 个文件）仍走 `apiV2`，拿到 v1 JWT 会在 `get_current_user` 处 40101，由 `auth:logout` 兜底重登：
