          ...
          <div class="banner-body">
            ...
            <div class="banner-meta">
              ...
              <span v-if="item.planned_delivery_date" class="delivery"
                >交期 {{ formatDate(item.planned_delivery_date) }}</span
              >
            </div>
          </div>
          <el-button link class="banner-close" @click="dismiss(item.key)"
...
function resumeTimer(item: BannerItem): void {
  if (items.value.some((i) => i.key === item.key)) scheduleDismiss(item.key);
}
...
.banner-picked_up .banner-icon {
  color: var(--primary-color);
}
.banner-placed_on_shelf {
  border-left-color: #e6a23c;
}
.banner-placed_on_shelf .banner-icon {
  color: #e6a23c;
}
.banner-released {
  border-left-color: #e6a23c;
}
.banner-released .banner-icon {
  color: #e6a23c;
}
.banner-returned {
  border-left-color: #67c23a;
}
.banner-returned .banner-icon {
  color: #67c23a;
}
.banner-inspected {
  border-left-color: #409eff;
}
.banner-inspected .banner-icon {
  color: #409eff;
}
/* 2026-08-05 召回 */
.banner-recalled {
