...
import { flattenCandidateBatches, filterTargetsByKind } from '../utils/scanCandidateFlatten';
import type {
  ScanAvailableBatch,
  ScanAttachableBatch,
...
describe('flattenCandidateBatches', () => {
  ...
  it('仅 B 组（inspectable）→ 全部 INSPECTABLE', () => {
    const t = mkTarget('p1', [], [{ batch_id: 'b1', quantity: 5, status: 'PENDING', version: 1 }]);
...
describe('filterTargetsByKind', () => {
  ...
  it('选中集合为空 → 返回 []', () => {
    const t1 = mkTarget('p1', [], [{ batch_id: 'b1', quantity: 2, status: 'PENDING', version: 1 }]);
