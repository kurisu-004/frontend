 ...
 */
export function parseBidExcel(workbook: XLSX.WorkBook, today: string): ParseResult {
  ...
  for (let i = 0; i < rawRows.length; i++) {
    ...
    // 整行空：静默跳过
    const allEmpty = [applicantName, deptCode, deptName, drawingNo, partName].every((s) => !s);
