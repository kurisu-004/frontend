...
export function canAddRemoveParts(status: DeliveryNoteStatus, role: RoleMapLike): boolean {
  if (!hasManageNoteRole(role)) return false;
  return status === 'DRAFT' || status === 'SUBMITTED';
}

export function canSubmit(status: DeliveryNoteStatus, role: RoleMapLike): boolean {
  return hasManageNoteRole(role) && status === 'DRAFT';
}

export function canRecall(status: DeliveryNoteStatus, role: RoleMapLike): boolean {
  return hasManageNoteRole(role) && status === 'SUBMITTED';
}

export function canSoftDelete(status: DeliveryNoteStatus, role: RoleMapLike): boolean {
  return hasManageNoteRole(role) && status === 'DRAFT';
}
