    ...
    <el-card shadow="never" class="filter-card">
      <el-form inline @submit.prevent="onSearch">
        ...
        <el-form-item label="状态">
          <el-select v-model="search.is_active" placeholder="全部" clearable style="width: 110px">
    ...
    >
      ...
      <el-table-column label="操作" min-width="220" fixed="right" align="center">
        <template #default="{ row }">
          <el-button link type="primary" size="small" @click="onEdit(row as Worker)"
            >编辑</el-button
          >
          <el-button
            ...
            >停用</el-button
          >
          <el-button v-else link type="success" size="small" @click="onReactivate(row as Worker)"
            >启用</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      <el-form ref="formRef" :model="form" label-width="80px">
        <el-form-item label="工牌码" required>
          <el-input v-model="form.badge_code" placeholder="工牌上的条码/二维码值" clearable />
        </el-form-item>
...
// ============ 列可见性 + 列顺序拖动 ============
// 「#」和「操作」列不放进 defs → 始终可见。
// 2026-08-27 T15：补 prop / width / minWidth / align + 复杂单元格走 cellRender(ListShell 同款)。
...
async function onDeactivate(row: Worker): Promise<void> {
  await ElMessageBox.confirm(`确认停用「${row.name}」？停用后该工牌将不能扫码领取。`, '提示', {
