  ...
  <div class="customer-list">
        ...
        <template #default="{ data }">
          <div class="tree-row">
            ...
            <span class="tree-row__actions">
              ...
              <el-button link type="primary" size="small" @click="onEdit(data as TreeNode)"
                >编辑</el-button
              >
              <el-button link type="danger" size="small" @click="onDelete(data as TreeNode)"
                >删除</el-button
              >
            </span>
          </div>
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="mode !== 'newRoot'" label="父客户" prop="parentId">
          <el-input v-if="mode === 'newChild'" :model-value="lockedParentName" disabled />
          <el-select
            v-else
        ...
        <el-form-item v-if="mode !== 'newChild'" label="序列号前缀" prop="serialPrefix">
          <el-select
            ...
            style="width: 100%"
          >
            <el-option v-for="p in serialPrefixOptions" :key="p" :label="p" :value="p" />
          </el-select>
          <p class="form-hint">
            一级客户的序列号前缀
            A-Z；新建后可在客户一览再次修改（仅影响后续创建的零件/装配体流水号）。
          </p>
        </el-form-item>
      </el-form>
      <template #footer>
...
const filteredTree = computed<TreeNode[]>(() => {
  ...
  const match = (n: { name: string }): boolean => n.name.toLowerCase().includes(kw);
  return tree.value
    .map((r) => {
...
const dialogTitle = computed(() =>
  mode.value === 'newRoot' ? '新增一级客户' : mode.value === 'newChild' ? '新增子客户' : '编辑客户',
);
...
async function onDelete(node: TreeNode): Promise<void> {
  ...
  const warn =
    childCount > 0
      ...
      : '';
  try {
    await ElMessageBox.confirm(`确认软删除客户「${node.name}」？${warn}`, '删除客户', {
