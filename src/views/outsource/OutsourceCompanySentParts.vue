...
const { restore: restoreSentPartsFilter } = useListStatePersist('outsource_company_sent_parts', {
  filter,
  sortBy,
  sortDir,
});
...
const columnDefs: ColumnDef[] = [
    ...
    cellRender: ({ row }) => {
      ...
      if (editingId.value === r.shipment_id) {
        return h(ElInputNumber, {
          modelValue: editBuffer.quantity,
          min: 1,
          max: 999999,
          controls: false,
          size: 'small',
          disabled: savingEdit.value,
          style: 'width: 80px',
          'onUpdate:modelValue': (v: number | null | undefined) => {
            editBuffer.quantity = v ?? null;
          },
        });
      }
  ...
  },
  {
    ...
    cellRender: ({ row }) => {
      ...
      if (editingId.value === r.shipment_id) {
        return h(ElInputNumber, {
          modelValue: editBuffer.unit_price,
          min: 0,
          precision: 2,
          step: 0.01,
          controls: false,
  ...
  },
  {
    ...
    align: 'center',
    cellRender: ({ row }) =>
      h(ElTag, { type: statusTagType((row as OutsourceSentPartItem).status), size: 'small' }, () =>
  ...
  },
  {
    ...
    cellRender: ({ row }) => {
      ...
      if (editingId.value === r.shipment_id) {
        return h(ElSwitch, {
          modelValue: editBuffer.is_billed,
          size: 'small',
          disabled: savingEdit.value,
...
function startEdit(row: OutsourceSentPartItem): void {
  ...
  editBuffer.unit_price = row.unit_price !== null ? Number(row.unit_price) : null;
...
onMounted(() => {
  ...
  if (persisted) {
    ...
    if (typeof persisted.sortBy === 'string')
      sortBy.value = persisted.sortBy as OutsourceSentPartSortKey;
  ...
  <div class="outsource-billing">
    <el-card shadow="never" class="filter-card">
      <div class="header-row">
        <h2 style="margin: 0; font-size: 16px">外协对账：{{ company?.name ?? '...' }}</h2>
