...
-->
<template>
  <el-card v-loading="quotesLoading" shadow="never" class="quote-card">
    <template #header>
    ...
    <el-table v-if="quotes.length > 0" ref="tableRef" :data="quotes" size="small" border stripe>
      <!--
    ...
    >
      <el-form ref="createFormRef" :model="createForm" :rules="quoteRules" label-width="100px">
        <el-form-item label="零件">
          <el-input :model-value="partName ?? ''" disabled placeholder="当前零件" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="createSubmitting" @click="onCreateConfirm"
          >保存为 DRAFT</el-button
        >
      </template>
    </el-dialog>
...
const columnDefs: ColumnDef[] = [
  {
    ...
    cellRender: ({ row }) => {
      ...
      const tagType = (OUTSOURCE_QUOTE_STATUS_TAG[r.status] || 'info') as
        'info' | 'success' | 'warning' | 'danger';
      return h(
        ElTag,
        { type: tagType, size: 'small', effect: 'plain' },
        () => OUTSOURCE_QUOTE_STATUS_LABEL[r.status],
      );
    },
  },
  {
  ...
  },
  {
    ...
    align: 'center',
    cellRender: ({ row }) =>
      h('span', { class: 'muted' }, formatDateTime((row as OutsourceQuote).created_at)),
  },
];
