...
<template>
  <div class="parts-list">
    <el-card shadow="never" class="filter-card">
      <div class="filter-row">
        ...
        <div class="filter-group filter-group--actions">
          ...
          <el-button v-if="canEdit" @click="router.push('/parts/new?tab=pdf')">
            <el-icon><Document /></el-icon>
            <span>从 PDF/Excel 批量导入</span>
          </el-button>

          <!-- 2026-08-12：采购订单 Excel 导入（解析系统交期和订单号；同 canEdit 闸门） -->
          <el-button v-if="canEdit" @click="orderImportVisible = true">
    ...
    <PurchaseOrderImportDialog v-model="orderImportVisible" @success="fetchList" />
  </div>
</template>
...
import { initialPartsSearch, usePartsListQuery } from './composables/usePartsListQuery';
...
import { ORDER_STATUS_LABEL, ORDER_STATUS_TAG_TYPE, PART_SORT_KEY_TO_PROP } from '@/types/parts';
