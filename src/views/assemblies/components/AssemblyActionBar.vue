...
<template>
  <div class="card-actions">
    ...
    <!-- 编辑元数据（CLERK+；终态由后端拒绝） -->
    <el-button v-if="canEditContent" type="primary" plain @click="emit('edit')">
      <el-icon><Edit /></el-icon>
      <span>编辑元数据</span>
    </el-button>
    <!-- 取消（CLERK+）：非终态可触发（serial 必须有） -->
    <el-button v-if="canCancel" type="warning" plain :disabled="!hasSerial" @click="emit('cancel')">
      <el-icon><CircleClose /></el-icon>
      <span>取消装配件</span>
    </el-button>
    <!-- 删除（MANAGER-only）：不论状态都可触发 -->
    <el-button v-if="canDelete" type="danger" plain :disabled="!hasSerial" @click="emit('delete')">
