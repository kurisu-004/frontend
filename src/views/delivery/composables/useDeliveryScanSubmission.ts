...
export function useDeliveryScanSubmission(opts: UseDeliveryScanSubmissionOptions) {
    ...
    switch (out.outcome) {
      case 'ADDED': {
        ...
        if (out.note) {
          ...
          l2Name = pickL2Name(detail?.line_items, (li) => String(li.id) === String(firstAddedId));
        }
        ElMessage.success(
      ...
      case 'ALREADY_PRESENT': {
        ...
        if (out.note) {
          const detail = await opts.refreshDraftDetail(out.note.id);
          l2Name = pickL2Name(detail?.line_items, (li) => li.serial_no === out.resolved?.serial_no);
        }
        ElMessage.warning(
      ...
      case 'CANDIDATES_AVAILABLE': {
        ...
        if (out.note) {
          ...
          l2Name =
            pickL2Name(items, (li) => li.serial_no === out.resolved?.serial_no) ??
      ...
      case 'PARTIAL_ADDED': {
        ...
        if (out.note) {
          ...
          l2Name = pickL2Name(detail?.line_items, (li) => addedIds.includes(String(li.id)));
        }
        candidateNoteId.value = out.note?.id ?? ''; // 2026-08-31 新增
        candidateTargets.value = out.unresolved_targets ?? [];
        originalScanCode.value = originalCode;
  ...
  async function confirmAndSubmit(d: ScanNoteSummary): Promise<void> {
    try {
      await ElMessageBox.confirm(`确认提交草稿 ${d.delivery_note_no}？`, '提交草稿', {
        type: 'warning',
  ...
  function onSubmitDraftError(e: unknown): void {
    ...
    if (err?.code === 21403 || err?.response?.data?.code === 21403) {
      ...
      if (target) {
        void opts.refreshDraftDetail(target.id).catch(() => {
          /* 已 toast */
        });
      }
      return;
  ...
  function onSubmitDialogPassPartial(result: {
    ...
    if (submitTarget.value) {
      void opts.refreshDraftDetail(submitTarget.value.id).catch(() => {
        /* 已 toast */
      });
    }
  }
