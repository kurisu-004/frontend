...
vi.mock('vue-draggable-plus', async () => {
  ...
  return {
    ...actual,
    useDraggable: <T>(
      _el: Ref<HTMLElement | null> | HTMLElement,
      _list: Ref<T[]>,
      opts: Record<string, unknown> & { immediate?: boolean },
    ) => {
...
describe('useColumnDrag 合并算法（onStart/onEnd + DOM）', () => {
  ...
  function makeMockEl(tagName: string, className = ''): MockEl {
    const el: MockEl = {
      tagName,
      className,
      children: [],
      ...
      querySelectorAll(selector: string): MockEl[] {
        ...
        return collectAll(el).filter((n) =>
          segs.every((part) => {
            ...
            if (cls && !n.className.split(/\s+/).filter(Boolean).includes(cls)) return false;
            return true;
          }),
        );
      },
  ...
  function buildHeaderTr(specs: Array<{ key?: string; literal?: boolean }>): MockEl {
    ...
    for (const s of specs) {
      if (s.literal) {
        ...
        tr.children.push(makeMockEl('th', `col-draggable col-drag-handle col-key-${s.key}`));
      }
    }
    return tr;
  }
  ...
  it('全部列可见：交换两列 → orderedKeys 与 localStorage 都得到新顺序', () => {
    ...
    expect(JSON.parse(localStorage.getItem('myerp.list.u1.swap_all_columnOrder')!)).toEqual([
      'b',
      'c',
      'a',
    ]);
  ...
  it('【回归点】有隐藏列时：DOM 只渲染部分列，拖动后隐藏列锚定在原槽位', () => {
    ...
    expect(JSON.parse(localStorage.getItem('myerp.list.u1.swap_hidden_columnOrder')!)).toEqual([
      'c',
      'b',
      'a',
      'd',
  ...
  it('dragKeys 长度与 subSet.size 不一致 → console.warn 且不写盘', () => {
    ...
    // 预存原值，便于断言「不被覆盖」
    localStorage.setItem('myerp.list.u1.mismatch_columnOrder', JSON.stringify(['PRE_EXISTING']));
...
describe('useColumnDrag 表头重建后自愈重绑（MutationObserver）', () => {
  ...
  function makeEl(tagName: string, className = '', isConnected = false): MockEl {
    const el: MockEl = {
      tagName,
      className,
      children: [],
  ...
  function buildMockElTable(opts: { initialTrs?: MockEl[] }): {
    root: MockEl;
    headerWrap: MockEl;
    bodyWrap: MockEl;
