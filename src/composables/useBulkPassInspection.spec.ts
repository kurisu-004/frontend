...
describe('toBatchPassItems', () => {
  ...
  it('quantity 缺省（undefined）保持 undefined', () => {
    const items: BulkPassItem[] = [{ batch_id: '190000000000030', version: 1, label: 'y' }];
    expect(toBatchPassItems(items)).toEqual([
  ...
  it('version 透传：version: 7 → 7', () => {
    const items: BulkPassItem[] = [{ batch_id: 'B-1', version: 7, label: 'L' }];
    expect(toBatchPassItems(items)).toEqual([{ batch_id: 'B-1', version: 7, quantity: undefined }]);
  });
});
...
describe('mapBatchResult', () => {
  ...
  it('防御占位 item 的 label 取 part.serial_no（2026-08-28 新增）', () => {
    ...
    const r = mapBatchResult(requested, out);
    expect(r.passed).toEqual([{ batch_id: '190000000000999', version: 0, label: 'S-999' }]);
  });
