...
> **核心价值**：14 业务域的总入口，给出文档链接、路由前缀、v1/v2 迁移状态
...
| 生产管理      | [production/README.md](production/README.md)               | `/production` `//workers/queue` | 工序制定 + 工序工种字典 + 生产队列看板               |
| 司机送货台    | [delivery-dispatch.md](delivery-dispatch.md)               | `/delivery-dispatch`            | 司机工牌识别 + 待送货单（独立全屏路由树）            |
| 装配          | [assemblies.md](assemblies.md)                             | `/assemblies/:id`               | 装配件详情（路由并入 `/parts`）                      |
| 打印模板      | [print-templates.md](print-templates.md)                   | `/print-templates`              | 通用表格编辑器（基于 x-data-spreadsheet，in-memory） |
