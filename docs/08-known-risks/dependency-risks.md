...
> 2026-09-14 起记录。新增依赖经过审查但仍需在生产运行 1 个月内重新评估「是否真的值得长期保留」，以下条目保留决策缘由方便回溯。

### 5.1 `@amdosion/vue3-print ^1.0.40`（2026-09-14 新增）

**用途**：`/print-templates` 打印模板编辑器底层渲染引擎，替换原自建的 `usePrintTemplates` + 8 个手动维护文件。

**决策缘由**：

- 维护成本收敛：8 个自建文件的拖拽 / 缩放 / 打印预览逻辑统一收到一个成熟 npm 包
- 依赖收敛：原本需要自己实现的元素选区、拖拽微调、打印坐标变换全部由包提供
- pinia `^3.0.4` 同步升级完成（2026-09-14 commit `092fcc2`）

**已知 trade-off**：

- 包大小增加约 80KB gzip（编辑器懒加载，业务页未引用）
- last commit 距今 < 1y（2025 Q4 仍在活跃），社区 star 较小（个人维护为主）—— 1 个月后重新评估 bus factor

**迁移路径**：若维护者停更，回退方案是把 `@amdosion/vue3-print` 的依赖图剪裁（只保留打印预览子模块），其余自建逻辑保留兜底。详见 [`docs/03-modules/print-templates.md`](../03-modules/print-templates.md)。
